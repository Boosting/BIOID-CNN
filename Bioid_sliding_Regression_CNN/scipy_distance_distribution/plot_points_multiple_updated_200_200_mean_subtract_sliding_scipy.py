...
for i in range(stride_no):
	for j in range(stride_no):
		...
		if i == 0 and j == 0:
			output_result_compiled = np.array(output_result)
...
output_result_compiled = output_result_compiled.reshape(stride_no*stride_no,4)
...
for j in range(0,2):
	Y = sc.spatial.distance.pdist(output_result_compiled[:,2*j:2*(j+1)], 'euclidean')
	Y = sc.spatial.distance.squareform(sc.spatial.distance.pdist(output_result_compiled[:,2*j:2*(j+1)], 'euclidean'))
	#print output_result_compiled[:,:2]

	print np.sort(Y)

	coordinate_distribution = []
	for i in range(stride_no * stride_no):
		print Y[i]
		coordinate_distribution.append(sum(i<5 for i in Y[i]))

	print coordinate_distribution
	max_index = coordinate_distribution.index(max(coordinate_distribution))
	print max_index
	print output_result_compiled[:,2*j:2*(j+1)][max_index]
	pl.scatter(output_result_compiled[:,2*j:2*(j+1)][max_index][0], output_result_compiled[:,2*j:2*(j+1)][max_index][1], marker='x', s=30)



#pl.show()
file_name = get_filename(file_name)
pl.savefig('sliding_window_distance_distribution_'+file_name+'.jpg',dpi=500)
