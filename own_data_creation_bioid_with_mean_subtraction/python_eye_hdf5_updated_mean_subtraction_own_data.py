...
def load(image_dir,eye_dir,min_index, max_index): 
	for i in range(min_index,max_index):
		for j in range(0,20):
			print i		
			number = str(i).zfill(4)
			image, eye_coordinate = load_single_img(os.path.join(image_dir,"BioID_" + number + "_" + str(j) + ".pgm"), os.path.join(image_dir,"BioID_" + number + "_" + str(j) + ".eye"))

			if i == min_index and j ==0:
	...
	image_compiled = image_compiled.reshape((max_index-min_index)*20,1,200,200)
	eye_coordinate_compiled = eye_coordinate_compiled.reshape((max_index-min_index)*20,4)
	print image_compiled.shape
...
image_compiled, eye_coordinate_compiled = load(image_dir, eye_dir, 0, 500)
...
sep = 9500
writeHdf5('train_scaled_mean_subtracted_own_data_0_500',image_compiled[0:sep],eye_coordinate_compiled[0:sep])
writeHdf5('val_scaled_mean_subtracted_own_data_0_500',image_compiled[sep:10000],eye_coordinate_compiled[sep:10000])

image_compiled, eye_coordinate_compiled = load(image_dir, eye_dir, 500, 1000)
# shuffle the image and eye coordinates to produce 2 sets for training and validation 
image_compiled, eye_coordinate_compiled = shuffle(image_compiled, eye_coordinate_compiled, random_state=42)
sep = 9500
writeHdf5('train_scaled_mean_subtracted_own_data_500_1000',image_compiled[0:sep],eye_coordinate_compiled[0:sep])
writeHdf5('val_scaled_mean_subtracted_own_data_500_1000',image_compiled[sep:10000],eye_coordinate_compiled[sep:10000])

image_compiled, eye_coordinate_compiled = load(image_dir, eye_dir, 1000, 1500)
# shuffle the image and eye coordinates to produce 2 sets for training and validation 
image_compiled, eye_coordinate_compiled = shuffle(image_compiled, eye_coordinate_compiled, random_state=42)
sep = 9500
writeHdf5('train_scaled_mean_subtracted_own_data_1000_1500',image_compiled[0:sep],eye_coordinate_compiled[0:sep])
writeHdf5('val_scaled_mean_subtracted_own_data_1000_1500',image_compiled[sep:10000],eye_coordinate_compiled[sep:10000])
